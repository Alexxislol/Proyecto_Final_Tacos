...
function getRowsInfo($string){
     ...
     foreach ($rst as $key){
        ...
        <tr id="trinsumesa'.$insData[0].'" class="trinsumolista"><td>'.$key['nombre'].'</td>
             <td id="priceIns'.$insData[0].'" data-price="'.$key['precio'].'">GTQ '.$key['precio'].'</td>
             <td>
             ...
             <td class="text-center subtotalpro" id="subTl'.$insData[0].'" data-subtl="'.$subtotal.'">GTQ '.$subtotal.'</td>
             <td>
               <div class="input-group mb-3">
...
function displayVentaData($accion){
   ...
   echo'
      ...
      <div class="col-md-3">
        ...
        <div class="card">
          <div class="card-body text-center">
              <h3>Pago con: <p>GTQ '.$key['pagocon'].'</p></h3>
          </div>
        </div>
      </div>
      <div class="col-md-3">
        ...
        <div class="card">
          <div class="card-body text-center">
              <h3>Cambio: <p>GTQ '.$key['cambio'].'</p></h3>
          </div>
        </div>
      </div>
      <div class="col-md-3">
...
function getRowsInfoTicket($string){
     ...
     foreach ($rst as $key){
        ...
        $dt = '
        '.$key['nombre'].' [GTQ '.$key['precio'].']  ['.$insData[1].'] ......... GTQ '.$ttlper.'<p></p>
        ';
     }
     return $dt;
}
...
<body>
  ...
  <p>Pago: GTQ '.$key['pagocon'].' - Cambio: GTQ '.$key['cambio'].'</p>
  <p>Folio: '.$key['id'].'</p>
  <script>
...
function getRowsInfoInfo($string){
     ...
     $dt = '';
     foreach ($rst as $key){ 
        $dt = '<p style="padding:0px;margin:0px;">'.$key['nombre'].' - ['.$insData[1].'] [GTQ '.$key['precio'].']</p>';
     }
     return $dt;
}
...
function getRowsInfoInfoFlat($string){
     ...
     $dt = '';
     foreach ($rst as $key){ 
        $dt = ''.$key['nombre'].' - ['.$insData[1].'] [GTQ '.$key['precio'].'] | ';
     }
     return $dt;
}
...
function createInfoPerMesa($mesa){
    ...
    foreach ($rst as $key){
       ...
       $xlsx->data('F'.$numi, 'GTQ '.$key['total']);
